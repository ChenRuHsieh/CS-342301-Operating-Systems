...
Thread::Thread(char* threadName, int threadID)
{
    ...
    name = threadName;
    //---------------------------------MP3-------------------------------------//
    priority = 149; //for handling system threads
    starttime = 0;
    predictburstTime = 10;
    burstTime = 0;
    agecount = 0;
...
Thread::Yield ()
{
    ...
    //---------------------------------MP3-------------------------------------//
    int nextThread_piority;
    int oldThread_piority;
    if (nextThread != NULL) {
        nextThread_piority = nextThread->getPriority();
        oldThread_piority = this->priority;
        if (oldThread_piority >=100 && oldThread_piority < 150) {
            if (nextThread_piority >= 100 && nextThread_piority < 150) {
                if (nextThread->getPredictBurstTime() <= (this->predictburstTime - this->burstTime)) {
                    kernel->scheduler->ReadyToRun(this);
                    kernel->scheduler->Run(nextThread, FALSE);
                } else {
        ...
        } else if (oldThread_piority >=50 && oldThread_piority < 100) {
            if (nextThread_piority >= 100 && nextThread_piority < 150) {
                kernel->scheduler->ReadyToRun(this);
                kernel->scheduler->Run(nextThread, FALSE);
            } else {
        ...
        } else if (oldThread_piority >=0 && oldThread_piority < 50) {
            if (nextThread_piority >= 100 && nextThread_piority < 150) {
                kernel->scheduler->ReadyToRun(nextThread);
            } else {
                kernel->scheduler->ReadyToRun(this);
...
Thread::Sleep (bool finishing)
{
    ...
    if (this->getPriority() >= 100) {
        this->calculateNextBurstTime();
    }
    // DEBUG(dbgOS, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getName() << "] is now selected for execution");
    // DEBUG(dbgOS, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getName() << "] is replaced, and it has executed [" << this->getBurstTime() << "] ticks");
    // this->setBurstTime(0);
    //---------------------------------MP3-------------------------------------//
