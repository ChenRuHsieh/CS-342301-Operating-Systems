...
Semaphore::P()
{
    ...
    while (value == 0) { 		// semaphore not available
        ...
        DEBUG(dbgOS, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << currentThread->getName() << "] is inserted into IO queue");
        currentThread->Sleep(FALSE); // if true, this Thread will be deleted 
    } 
    value--; 			// semaphore available, consume its value
   
    // re-enable interrupts
    DEBUG(dbgTraCode, "end sleep");
    (void) interrupt->SetLevel(oldLevel);	
    DEBUG(dbgTraCode, "esadsadsadsadsa");
}
...
Semaphore::V()
{
    ...
    if (!queue->IsEmpty()) {  // make thread ready.
        ...
        piority = kernel->currentThread->getPriority(); // old T priority
        tmp = queue->RemoveFront(); // new thread
        DEBUG(dbgOS, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << tmp->getName() << "] is removed from IO queue");
        tmp_piority = tmp->getPriority(); // new T priority
        if (tmp_piority >= 100 && tmp_piority < 150) { // new T is L1
            kernel->scheduler->ReadyToRun(tmp);
            kernel->scheduler->setPreemptive(TRUE);
        } else { // new T isn't L1
            kernel->scheduler->ReadyToRun(tmp);
            kernel->scheduler->setPreemptive(FALSE);
        }
